...
3. **Validate specs.** `plans/<ticket>/architecture.md` exists; `plans/<ticket>/e2e/*.feature` exists; every in-scope role (`backend` and/or `frontend`) has `plans/<ticket>/<role>.spec.md`. Specs must **not** contain source file lists. Each `dependsOn` must match a contract section in `architecture.md`.
...
8. **Print start commands** if standalone `/awe-approve`: `/awe-code backend` and/or `/awe-code frontend`. `/awe-run` continues to `/awe-code`. Coding agents still write their own implementation plans and may stop for **their** questions before any source lands.
