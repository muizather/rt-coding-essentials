...
6. **Spawn `awe-architect`** with: ticket id, intake + open-questions paths, discovered roles (`backend` / `frontend` / `fullstack`), any `docs/awe/` paths that exist, instruction that **file lists and unit tests are forbidden**, gherkin is E2E only, specs go in `plans/<ticket>/<role>.spec.md`. A Magento repo with a sibling Next.js SPA is **not** fullstack.
