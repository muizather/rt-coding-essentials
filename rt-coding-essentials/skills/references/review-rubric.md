...
1. **Correctness** — does it do what it claims? Match the spec/plan and acceptance criteria; edge cases (null, empty, boundary); error paths not just the happy path; tests actually test the right things; no off-by-one, race, or state inconsistency; coder-written e2e specs map 1:1 to the gherkin scenarios and visibly demonstrate the outcome on screen.
