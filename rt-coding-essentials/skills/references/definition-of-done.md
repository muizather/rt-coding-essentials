...
<!-- Source: references/definition-of-done.md. In AWE this is the standing bar the stop-hook enforces via awe-evidence.json. -->
...
In AWE this is the **evidence bar**: the `stop` hook will not let a code/review session end until `awe-evidence.json` proves the Correctness items, and the ship gate won't open until Ship-readiness is met. The `10-awe-phases.mdc` rule points here as the standing reference.
...
The depth behind these items lives in the five-axis review (`review-rubric.md`) and in behavior-preserving simplification (Chesterton's Fence — see `awe-review`).
...
- [ ] Architectural decisions worth preserving are recorded as ADRs (see `awe-ship` docs gate)
- [ ] Documentation describes the current state in timeless language, not the change history

### Ship-readiness
- [ ] Security implications reviewed for any untrusted input, auth, or data handling (see `security-review-checklist.md`)
- [ ] Observability in place for new critical paths (logs, metrics, traces)
- [ ] Rollback path exists for anything risky (see `ship-decision.md`)
...
- **Per release**: the full checklist is the floor; `awe-ship` adds the deploy-specific gates (Ship Decision artifact, rollback plan, error-budget check) on top.

Tailor the list to the project once, then reuse it unchanged. A Definition of Done that is renegotiated every sprint is not a Definition of Done. The numeric floors live in `CONSTRAINTS.md`; this file is the *process* bar, that one is the *measurable* bar.
