---
name: awe-review
description: Review one role's diff — scanners, adversarial functional review, optional security reviewer, verdict handling with iteration budget. Usage: /awe-review <role>
---
...
6. **Collect verdicts** from `plans/<ticket>/reviews/<role>-functional-round-<N>.md` (and `<role>-security-round-<N>.md` when the optional agent ran). Merge into `plans/<ticket>/reviews/round-<N>.md` (scanner output + verdict(s) + combined findings table). Lead with Critical/Required; don't bury a real issue under cosmetics.
6. **Act on the verdict** (needs-fix from the functional reviewer, or from the security reviewer when `securityReview` is on). **A single Critical finding fails the iteration** — severity labels decide what burns a round: Critical/Required → needs-fix; Optional/Nit/FYI never block alone.
   ...
   - **verified from the required reviewer(s)** → set `roles.<role>.verified: true`, `planStatus` stays `approved`. When **all** roles are verified → set `phase: verify` and continue to `/awe-verify` when this is `/awe-run`, otherwise tell the human to run `/awe-verify`. Otherwise report which roles remain. Follow `references/mcp-report.md` if Slack/GitHub/GitLab tools exist.
...
5. **STOP** — the loop is **bounded by `reviewIterations`** (default 3), not infinite. A `verified` verdict from the required reviewer(s) stops it; so does the budget. Where the original skill escalates to a *cross-model CLI second opinion*, AWE escalates to a **human** instead — three unresolved rounds write `ESCALATION.md` and hand the decision to a person. (Per the Step-3 model constraint, AWE pins Cursor-subscription models and never invokes third-party CLI reviewers.)
