...
1. State `active: true`, `phase: ship`.
2. `.cursor/state/awe-signoff.json` exists with `verified: true` (human verify gate).
3. `.cursor/state/awe-evidence.json` has `testsPassed: true` and `at` younger than 2 hours — stale? Re-run the configured test command in each role worktree and refresh evidence first.
...
1. **Commit** any remaining changes in each role worktree with a conventional message referencing the ticket (`feat(PROJ-123): …`).
